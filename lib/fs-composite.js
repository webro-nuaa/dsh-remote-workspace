...
export default class RemoteWorkspaceFileSystem extends SandboxedFileSystem {
  ...
  isRemoteTarget(target) {
    return target != null && typeof target.targetKey === 'string' && target.targetKey.startsWith(REMOTE_SCHEME);
  }
  splitKey(targetKey) {
    const rest = targetKey.slice(REMOTE_SCHEME.length);
  ...
  async resolveAnchored(path, opts) {
    ...
    for (const anchor of anchors) {
      ...
      return this.resolve(REMOTE_SCHEME + anchor.connection + '/' + remotePath.replace(/^\//, ''), opts);
    }
    return undefined;
  }
  async resolve(path, opts) {
    if (typeof path === 'string' && path.startsWith(REMOTE_SCHEME)) {
      ...
      return {
        targetKey: FsTargetKey(REMOTE_SCHEME + name + '/' + rpath.slice(1)),
  ...
  fileUrl(target) {
    if (this.isRemoteTarget(target)) {
      throw new FsError(`"${target.displayPath}" lives on a remote workspace and has no local file URL`, 'FS_IO_ERROR');
    }
  ...
  async lstat(path, opts, signal) {
    ...
    if (typeof path !== 'string' || !path.startsWith(REMOTE_SCHEME)) return super.lstat(path, opts, signal);
  ...
  async listDir(target, signal) {
    ...
    const childKey = (childName) => REMOTE_SCHEME + name + '/' + childPath(childName).slice(1);
    return r.entries
      .slice()
