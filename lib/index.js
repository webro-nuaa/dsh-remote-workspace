  ...
  function installWebSurface() {
    ...
    disposers.push(webServer.register({
      ...
      async handler(req, res) {
        ...
        await saveProfile({ ...profile, remoteRoot });
        const anchorDir = await ensureAnchor(profileName, { remoteRoot });
        const registry = ctx.get('workspaceRegistry');
        if (!registry) { sendJson(res, 503, { error: 'workspace registry unavailable' }); return; }
        const title = String(body.title || '') || `[remote] ${profileName}:${remoteRoot}`;
        try {
