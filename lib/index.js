...
const BUNDLED_DAEMON_PATH = join(PACKAGE_DIR, '..', 'daemon', 'dsh-remote-daemon.js');
...
export function apply(ctx, config) {
  ...
  async function doConnect(args) {
    ...
    // 2. node availability
    const nodeCheck = await runCollect(sshArgv(spec, 'command -v node >/dev/null 2>&1 && node -v || echo __NO_NODE__'), { env });
    ...
    const remoteV = await runCollect(sshArgv(spec, 'node ~/.dsh-remote/daemon.js --version 2>/dev/null || true'), { env });
    ...
    handle = spawnProc(sshArgv(spec, 'exec node ~/.dsh-remote/daemon.js'), { env, raw: true });
    const rpc = new RpcClient(handle.stdin, handle.stdout);
