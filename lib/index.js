...
import { anchorDirFor, ensureAnchor } from './anchors.js';
...
export function apply(ctx, config) {
  ...
  // The composite fs row (dsh-remote-workspace/fs) resolves this service lazily
  // per call, so activation order between the rows never matters.
  ctx.provide('remote-workspace', {
    connectionFor(name) { return conns.get(name); },
    async anchors() {
      const profiles = await listProfiles();
      return profiles
        .filter((p) => p.remoteRoot)
        .map((p) => ({ connection: p.name, remoteRoot: p.remoteRoot, anchorDir: anchorDirFor(p.name) }));
    },
  });

  // Self-healing: the anchor directories backing registered remote workspaces
  // must always exist, or the stock registry reports the workspace as missing.
  // Idempotent — existing dirs are left untouched.
  async function repairAnchors() {
    try {
      const profiles = await listProfiles();
      for (const p of profiles) {
        if (p.remoteRoot) await ensureAnchor(p.name, { remoteRoot: p.remoteRoot });
      }
    } catch (error) {
      console.error('dsh-remote-workspace: anchor repair failed:', String(error && error.message ? error.message : error));
    }
  }
  repairAnchors();

  // ---- credential-backed auto-reconnect ----
  // Saved passwords live in the DSH credential store (~/.dsh/.credentials.yaml,
  // owner-only file, the same store the Models page writes API keys into), keyed
  // per CONNECTION name. At Host startup every saved profile reconnects
  // automatically, so remote workspaces survive restarts transparently.
  function credentialRefFor(connectionName) {
    const raw = String(connectionName || 'default');
    const ascii = raw.replace(/[^A-Za-z0-9_]/g, '');
    const hash = createHash('sha1').update(raw).digest('hex').slice(0, 8);
    return `dsh_rw_ssh_${ascii.length > 0 ? ascii + '_' : ''}${hash}`;
  }
    ...
    try {
      ...
      await Promise.allSettled(profiles.map((p) =>
        doConnect({
          ...
          auth: p.auth || 'key', keyPath: p.keyPath, transport: 'ssh',
        }).then(
          () => console.log('dsh-remote-workspace: auto-reconnected ' + p.name),
          (error) => console.error('dsh-remote-workspace: auto-reconnect failed for ' + p.name + ': ' + (error && error.message ? error.message : error)),
        )));
    } catch (error) {
      console.error('dsh-remote-workspace: auto-reconnect pass failed:', String(error && error.message ? error.message : error));
    }
  }
  autoReconnect();
  ...
  async function doConnect(args) {
    ...
    if (spec.auth === 'password') {
      if (!spec.password) {
        // Fall back to the stored credential so restarts reconnect without
        // retyping (auto-reconnect uses this path too).
        const svc = credentialsSvc();
        const stored = svc ? await svc.resolve(credentialRefFor(name)).catch(() => undefined) : undefined;
        if (stored && stored.value) spec.password = stored.value;
      }
      if (!spec.password) throw new Error('password required for auth=password（输入一次并勾选「记住密码」，之后随 Host 启动自动重连）');
    ...
    conns.set(name, { name, spec, handle, rpc, ping });
    await repairAnchors(); // the connected profile may own a workspace anchor
    return summarize(conns.get(name));
  }
  ...
  function installWebSurface() {
    ...
    disposers.push(webServer.register({
      ...
      async handler(req, res) {
        ...
        if (req.method === 'DELETE') {
          ...
          if (removed) {
            const svc = credentialsSvc();
            if (svc) await svc.unset(credentialRefFor(String(body.name || ''))).catch(() => {});
          }
    ...
    disposers.push(webServer.register({
      ...
      async handler(req, res) {
        ...
        try {
          ...
          if (body.remember && body.password && spec.auth === 'password') {
            const svc = credentialsSvc();
            if (svc) await svc.set(credentialRefFor(String(body.name || 'default')), body.password).catch((error) => {
              console.error('dsh-remote-workspace: credential store failed:', String(error && error.message ? error.message : error));
            });
          }
    ...
    disposers.push(webServer.register({
      path: '/plugins/dsh-remote-workspace/repair',
      async handler(req, res) {
        if (req.method !== 'POST') { sendJson(res, 405, { error: 'POST required' }); return; }
        await repairAnchors();
    ...
    // Sidebar "add workspace" flow: validate the remote root, persist it on the
    // profile, create the anchor dir, and return the anchor path. The actual
    // workspace registration rides the host adoption path (the flow reports
    // anchorDir via onPicked; workspaces.create canonicalizes and dedupes).
    disposers.push(webServer.register({
      path: '/plugins/dsh-remote-workspace/prepare',
      async handler(req, res) {
        if (req.method !== 'POST') { sendJson(res, 405, { error: 'POST required' }); return; }
        const body = await readJsonRequest(req);
        const profileName = String(body.profile || '');
        const remoteRoot = String(body.remoteRoot || '').trim();
        if (!profileName || !remoteRoot.startsWith('/')) {
          sendJson(res, 400, { error: 'profile and an absolute remoteRoot (starting with /) are required' });
          return;
        }
        const profiles = await listProfiles();
        const profile = profiles.find((p) => p.name === profileName);
        if (!profile) { sendJson(res, 404, { error: 'no saved profile named ' + profileName }); return; }
        const root = remoteRoot.replace(/\/+$/, '') || '/';
        const conn = conns.get(profileName);
        if (!conn || !conn.rpc) {
          sendJson(res, 400, { error: `连接「${profileName}」未激活：请先连接` });
          return;
        }
        const st = await conn.rpc.request('fs.stat', { path: root }).catch(() => null);
        if (!st || !st.isDir) {
          sendJson(res, 400, { error: `远端路径不存在或不是目录：${root}` });
          return;
        }
        await saveProfile({ ...profile, remoteRoot: root });
        const anchorDir = await ensureAnchor(profileName, { remoteRoot: root });
        sendJson(res, 200, { ok: true, anchorDir, remoteRoot: root });
      },
    }));

    // Remote directory browser: lists directories at `path` (default: the
    // remote home from the daemon ping) so the settings page can offer a
    // pick-the-path UX instead of typing absolute paths blind.
    disposers.push(webServer.register({
      path: '/plugins/dsh-remote-workspace/browse',
      async handler(req, res) {
        if (req.method !== 'POST') { sendJson(res, 405, { error: 'POST required' }); return; }
        const body = await readJsonRequest(req);
        const name = String(body.connection || '');
        const conn = conns.get(name);
        if (!conn || !conn.rpc) {
          sendJson(res, 400, { error: `连接「${name}」未激活：请先连接，再浏览远端目录` });
          return;
        }
        const home = (conn.ping && conn.ping.home) || '/';
        const path = String(body.path || '').trim() || home;
        try {
          const st = await conn.rpc.request('fs.stat', { path }).catch(() => null);
          if (!st || !st.isDir) {
            sendJson(res, 400, { ok: false, error: `远端路径不存在或不是目录：${path}`, home });
            return;
          }
          const r = await conn.rpc.request('fs.list', { path });
          ...
          const parent = path !== '/'
            ...
            : null;
          sendJson(res, 200, { ok: true, path, home, parent, entries });
        } catch (error) {
          sendJson(res, 400, { ok: false, error: `无法访问远端路径 ${path}：` + String(error && error.message ? error.message : error), home });
        }
      },
    }));

    disposers.push(webServer.register({
      path: '/plugins/dsh-remote-workspace/workspace',
      async handler(req, res) {
        if (req.method !== 'POST') { sendJson(res, 405, { error: 'POST required' }); return; }
        const body = await readJsonRequest(req);
        const profileName = String(body.profile || '');
        ...
        const profile = profiles.find((p) => p.name === profileName);
        if (!profile) { sendJson(res, 404, { error: 'no saved profile named ' + profileName }); return; }
        // Guard: the remote root must exist and be a directory BEFORE anything
        // is persisted or registered — a typo must never become a workspace.
        const root = remoteRoot.replace(/\/+$/, '') || '/';
        const conn = conns.get(profileName);
        if (!conn || !conn.rpc) {
          sendJson(res, 400, { error: `连接「${profileName}」未激活：请先在上方连接（挂载前需要校验远端路径）` });
          return;
        }
        const st = await conn.rpc.request('fs.stat', { path: root }).catch(() => null);
        if (!st || !st.isDir) {
          sendJson(res, 400, { error: `远端路径不存在或不是目录：${root}（工作区未创建）` });
          return;
        }
        // Persist the mapping, create the anchor dir, register the workspace.
        await saveProfile({ ...profile, remoteRoot: root });
        const anchorDir = await ensureAnchor(profileName, { remoteRoot: root });
        const registry = ctx.get('workspaceRegistry');
        if (!registry) { sendJson(res, 503, { error: 'workspace registry unavailable' }); return; }
        const title = String(body.title || '') || `[remote] ${profileName}:${root}`;
        try {
          const workspace = await registry.create(anchorDir, title);
          sendJson(res, 200, {
            ok: true,
            workspace: { id: workspace.id, path: workspace.path, title: workspace.title },
            anchorDir,
            hint: '工作区已加入左侧列表；打开它即可像本地工作区一样使用（文件树/编辑/命令都落在远端）',
          });
        } catch (error) {
          sendJson(res, 400, { ok: false, error: String(error && error.message ? error.message : error) });
        }
