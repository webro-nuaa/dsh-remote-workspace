// Host-side prototype plugin for dsh-remote-workspace.
// Mirrors the running cordis package rmws-1 (keep in sync; this file is the
// version-controlled source of truth for the prototype).
return {
  inject: ['timer'],
  apply(ctx) {
    const subprocess = ctx.get('subprocess');
    const fsSvc = ctx.get('fs');
    if (subprocess === undefined) throw new Error('dsh-remote-workspace: subprocess service unavailable');

    const DEFAULT_DAEMON_PATH = 'D:/code/ai_workspace/dsh-remote-workspace/daemon/dsh-remote-daemon.js';
    const conns = new Map();
    let connSeq = 0;
    ...
    async function readDaemonSource(daemonPath) {
      if (fsSvc === undefined) throw new Error('fs service unavailable');
      const target = await fsSvc.resolve(daemonPath);
    ...
    function spawnProc(argv, opts) {
      ...
      return subprocess.spawn({
        argv,
        cwd: opts.cwd || 'D:/code/ai_workspace',
        env: opts.env,
        graceMs: 5000,
        stdio: {
    ...
    RpcClient.prototype.request = function (method, params, timeoutMs) {
      ...
      return new Promise((res, rej) => {
        ...
        try { this.stdin.write(JSON.stringify({ id: id, method: method, params: params || {} }) + '\n'); }
    ...
    async function writeAskpass() {
      if (fsSvc === undefined) throw new Error('fs service unavailable (needed for password askpass)');
      const rel = '.dsh-remote-askpass-' + (++connSeq) + '.cmd';
      const target = await fsSvc.resolve(rel);
      await fsSvc.writeText(target, '@echo %DSH_RW_PW%\r\n');
      return rel;
    }
    ...
    async function doConnect(args) {
      ...
      const probe = await runCollect(sshArgv(spec, 'echo __DSH_RW_OK__'), { env: env });
      ...
      const nodeCheck = await runCollect(sshArgv(spec, 'command -v node >/dev/null 2>&1 && node -v || echo __NO_NODE__'), { env: env });
      if (nodeCheck.out.indexOf('__NO_NODE__') >= 0) throw new Error('remote host has no node on PATH (daemon needs node >= 16)');

      const remoteV = await runCollect(sshArgv(spec, 'node ~/.dsh-remote/daemon.js --version 2>/dev/null || true'), { env: env });
      if (remoteV.out.trim() !== daemonVersion) {
        const up = await runCollect(
          sshArgv(spec, 'mkdir -p ~/.dsh-remote && cat > ~/.dsh-remote/daemon.js.tmp && mv ~/.dsh-remote/daemon.js.tmp ~/.dsh-remote/daemon.js && node ~/.dsh-remote/daemon.js --version'),
          { env: env, input: daemonSource });
      ...
      handle = spawnProc(sshArgv(spec, 'exec node ~/.dsh-remote/daemon.js'), { env: env, raw: true });
      const rpc = new RpcClient(handle.stdin, handle.stdout);
    ...
    async function doExec(args) {
      ...
      const start = await conn.rpc.request('exec.start', { argv: argv, cwd: args.cwd });
      const timeoutMs = args.timeoutMs || 60000;
      const r = await conn.rpc.request('exec.wait', { jobId: start.jobId, timeoutMs: timeoutMs }, timeoutMs + 15000);
    ...
    // base64 helpers (no Buffer in the dynamic host sandbox).
    ...
    const textOut = { schema: { type: 'string' }, render: function (_a, v) { return [{ type: 'text', text: String(v) }]; } };
    function def(name, description, parameters, execute) {
      harness.registerTool(ctx, harness.defineTool({ name: name, description: description, parameters: parameters, output: textOut, execute: execute }));
    }
    ...
    def('remote_connect',
      'Connect a DSH remote workspace: bootstrap the executor daemon on the target and open the RPC channel. transport=direct spawns the daemon locally for testing (no SSH).',
      {
        ...
        daemonPath: { type: 'string', description: 'local path of the daemon source to bootstrap; has a default' },
      },
      doConnect);
    ...
    def('remote_exec',
      'Run a bash command in the remote workspace (via daemon exec.start/exec.wait) and return output.',
      {
        command: { type: 'string', description: 'bash command to run remotely' },
        argv: { type: 'string', description: 'OR explicit argv array (skips the bash -c wrapper); one of command/argv required' },
        name: { type: 'string', description: 'connection name, defaults to "default"' },
    ...
    console.log('dsh-remote-workspace prototype: tools registered (remote_connect/disconnect/status/exec/fs)');
  },
}
